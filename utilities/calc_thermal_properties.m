function [k_model, cp_model, rho_model] = calc_thermal_properties()
    ...
    K_model = [repmat(Ks(1),1,3),K_equ]; % used in the model
    ...
    mass = 0;
    for i = 1:4
        rho_equ = rho_equ + 4/3*pi*(Rs(i+2)^3 - Rs(i+1)^3)*rhos(i+1);
        vol = vol + 4/3*pi*(Rs(i+2)^3 - Rs(i+1)^3);
    end
    rho_equ = rho_equ/vol
    rhos =[repmat(rhos(1),1,3), rho_equ];
    ...
    mass = 0;
    for i = 1:4
        cp_equ = cp_equ + 4/3*pi*(Rs(i+2)^3 - Rs(i+1)^3)*rhos(i+1)*cps(i+1);
        mass = mass + 4/3*pi*(Rs(i+2)^3 - Rs(i+1)^3)*rhos(i+1);
    end
    cp_equ = cp_equ/mass
    
    cp_model=[repmat(cps(1), 1, 3), cp_equ];
