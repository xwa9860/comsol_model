...
loadSS1FromFile = false;
if loadSS1FromFile
     model = mphload([output_path, 'ss1.mph']);
else
    loadSSFromFile = true;
    if loadSSFromFile
        fprintf('loading steady state results from file ss.mph');
        model = mphload([output_path, 'ss.mph']);
    else
        fprintf('running the solvers from the beginning');
        loadEigenFromFile = true;
    ...
    while abs(new_eigen - lambda_eigen) > 0.001        
        model = run_steady_state_solver(model, lambda_eigen, 'ss1.mph');    
        % Rerun eigenvalue calculation with temperature profile from steady state
        lambda_eigen = new_eigen;
        [model, new_eigen] = run_eigen_solver(model, 'eigen1.mph', isInitialRun);
    end
end
...
while abs(new_eigen - target_eigen) > 10^-3     
    ...
    [model, new_eigen] = run_eigen_solver(model, 'eigen_cr.mph', false);
    % if new_eigen > target_eigen, keff < target_keff, need less control
    % rods insertion, need to increase the rod_height
    % The proportional controller 40 is chosen based on control rods
    % worth in this height region. 
    rod_height = rod_height + 30 * (new_eigen - target_eigen)
end
